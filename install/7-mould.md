...
sed -i -r 's#4096#65535#g' /etc/security/limits.d/20-nproc.conf
...
``` shell
sed -i -r 's#(.*) / (.*)defaults(.*)#\1 / \2defaults,nobarrier,noatime\3#g' /etc/fstab
...
sed -i -r '/vmlinuz-3.10.0/s#(.*)#\1 numa=off#g' /boot/grub2/grub.cfg
...
sed -i -r "s#IPADDR=192\.168\.15\.(.*)#IPADDR=192.168.15.\${IP}#g" /etc/sysconfig/network-scripts/ifcfg-ens33
...
``` shell
echo "* * * * * root /usr/sbin/ntpdate cn.pool.ntp.org > /dev/null 2>&1" >> /etc/crontab
